...
def load_ensemble_config_from_pipeline_results(symbol):
    ...
    if symbol in loaded_ensemble_configs:
        return loaded_ensemble_configs[symbol]

    if not os.path.exists(PIPELINE_RESULTS_PATH):
        logger.warning(f"Arquivo de resultados do pipeline não encontrado: {PIPELINE_RESULTS_PATH}")
        return None
    try:
        ...
        if not symbol_results:
            logger.warning(f"Resultados para {symbol} não encontrados em {PIPELINE_RESULTS_PATH}")
            return None
        ...
        loaded_ensemble_configs[symbol] = config
        logger.info(f"Configurações de ensemble para {symbol} carregadas: {config}")
        return config
    except Exception as e:
        logger.error(f"Erro ao carregar configurações de ensemble para {symbol}: {e}")
        return None

def load_hall_of_fame_strategies(symbol):
    """Loads Hall of Fame strategies for a given symbol."""
    if symbol in loaded_hall_of_fame:
        return loaded_hall_of_fame[symbol]

    hall_of_fame_file = HALL_OF_FAME_PATH_TEMPLATE.format(symbol)
    if not os.path.exists(hall_of_fame_file):
        logger.warning(f"Hall of Fame não encontrado para {symbol}: {hall_of_fame_file}")
        return []
    try:
        ...
        for ind in loaded_pop:
            ...
            seed_pop.append(ind)
        loaded_hall_of_fame[symbol] = seed_pop
        logger.info(f"Hall of Fame para {symbol} carregado: {len(seed_pop)} estratégias.")
        return seed_pop
    except Exception as e:
        logger.error(f"Erro ao carregar Hall of Fame para {symbol}: {e}")
        return []

def carregar_modelo_IA(symbol):
    if symbol in loaded_ml_models:
        return loaded_ml_models[symbol]
    try:
        p = model_path_for(symbol)
        if not p.exists():
            logger.info(f"Modelo não encontrado para {symbol}: {p}")
            return None
        model = joblib.load(p)
        logger.info(f"Modelo carregado: {p}")
        return model
    except Exception as e:
        logger.error(f"Erro carregar_modelo_IA({symbol}): {e}")
        return None    

def carregar_features_IA(symbol):
    if symbol in loaded_ml_features:
        return loaded_ml_features[symbol]
    try:
        p = features_path_for(symbol)
        if not p.exists():
            logger.info(f"Arquivo de features não encontrado para {symbol}: {p}")
            return None
        with open(p, 'r') as f:
        ...
        if not features:
            ...
            return None
        loaded_ml_features[symbol] = features
        logger.info(f"Features para {symbol} carregadas: {len(features)} features.")
        return features
    except Exception as e:
        logger.error(f"Erro ao carregar features para {symbol}: {e}")
        return None
...
def gerar_sinal_IA(simbolo, timeframe, lookback=200): # Removed melhores_params
    ...
    if sinal_bloqueado():
        ...
        return None, None, 0.0, 2.0, 4.0, 0.0
    ...
    if rates is None or len(rates) < 100: # Need enough data for feature calculation
        return None, None, 0.0, best_sl_atr, best_tp_atr, 0.0
    ...
    df = df[['open','high','low','close','tickvol']].copy() # Keep tickvol for VWAP
    ...
    if not ml_features:
        logger.error(f"Features para ML não carregadas para {simbolo}. Usando fallback.")
        ...
        return ("COMPRA" if final_signal_value == 1 else "VENDA" if final_signal_value == -1 else None), float(df['close'].iloc[-1]), confianca, best_sl_atr, best_tp_atr, df['atr_14'].iloc[-1]
    ...
    if modelo is not None:
        ...
        logger.warning(f"Modelo ML não carregado para {simbolo}. Usando apenas sinal do ensemble.")
    ...
    if risco_noticias > 0.5:
        ...
        return None, float(df['close'].iloc[-1]), 0.0, best_sl_atr, best_tp_atr, df['atr_14'].iloc[-1]
    ...
    if mercado_aberto and sinal is not None and confianca > 0.5: # Only trade if confident
        ...
        if ok:
            ...
            return sinal, ultimo_preco, confianca, best_sl_atr, best_tp_atr, df['atr_14'].iloc[-1]

    return None, ultimo_preco, 0.0, best_sl_atr, best_tp_atr, df['atr_14'].iloc[-1]
...
if __name__ == "__main__":
    if initialize_mt5():
        # Exemplo de uso para um símbolo
        sinal, preco, confianca, sl_atr, tp_atr, atr_val = gerar_sinal_IA("WIN$", mt5.TIMEFRAME_M5, lookback=500)
